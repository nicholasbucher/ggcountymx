...
#' @examples
#' ggcountymx("newyork")
ggcountymx <- function(state="florida", fips=FALSE, fill="white",
  ...
  gg <- ggplot()
  cnty.geom <- geom_map(data=cty.f.f, map = cty.f.f, aes(map_id=id, x=long, y=lat),
...
#' @examples
#' g <- ggcounty.us()
ggcounty.mx <- function(fill="white", color="#7f7f7f", size=0.25, alpha=1) {
  ...
  us.file <- system.file(package="ggcountymx", "counties", "48.shp")
